...
export default async function AdminContactsPage(props: { searchParams: Promise<{ page?: string }> }) {
  ...
  const [countResult] = await db.select({ value: count() }).from(contacts);
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '800px' }}>
            <thead>
              <tr style={{ 
                ...
                <th style={{ padding: '16px 24px', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '1.5px', color: 'rgba(0,0,0,0.45)', fontWeight: 600 }}>Name / Email / Phone</th>
                <th style={{ padding: '16px 24px', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '1.5px', color: 'rgba(0,0,0,0.45)', fontWeight: 600 }}>Subject</th>
                <th style={{ padding: '16px 24px', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '1.5px', color: 'rgba(0,0,0,0.45)', fontWeight: 600 }}>Message</th>
            ...
            <tbody>
              {paginatedContacts.map((c, index) => {
                const isLast = index === paginatedContacts.length - 1;
                
                return (
                  <tr key={c.id} className="admin-table-row" style={{ 
                    ...
                    <td style={{ padding: '16px 24px' }}>
                      <div style={{ fontWeight: 600, color: 'var(--charcoal, #2a3a30)', fontSize: '0.9rem' }}>{c.name}</div>
                      <div style={{ fontSize: '0.75rem', color: 'rgba(0,0,0,0.5)' }}>{c.email}</div>
                      {c.phone && <div style={{ fontSize: '0.75rem', color: 'rgba(0,0,0,0.5)', marginTop: '2px' }}>{c.phone}</div>}
                    </td>
        ...
        {totalCount > 0 && (
          <div style={{ 
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              {page > 1 ? (
                <Link href={`/admin/dashboard/contacts?page=${page - 1}`} className="admin-page-btn" style={{
                  padding: '6px 12px',
                  fontSize: '0.8rem',
                  fontWeight: 500,
                  color: 'var(--charcoal, #2a3a30)',
