...
export default async function AdminReviewsPage(props: { searchParams: Promise<{ page?: string }> }) {
  ...
  const [countResult] = await db.select({ value: count() }).from(reviews);
  const totalCount = countResult.value;
  const totalPages = Math.ceil(totalCount / pageSize);

  const paginatedReviews = await db.query.reviews.findMany({
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '800px' }}>
            <thead>
              <tr style={{ 
                ...
                <th style={{ padding: '16px 24px', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '1.5px', color: 'rgba(0,0,0,0.45)', fontWeight: 600 }}>Reviewer</th>
                <th style={{ padding: '16px 24px', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '1.5px', color: 'rgba(0,0,0,0.45)', fontWeight: 600 }}>Rating / Project</th>
                <th style={{ padding: '16px 24px', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '1.5px', color: 'rgba(0,0,0,0.45)', fontWeight: 600 }}>Review</th>
            ...
            <tbody>
              {paginatedReviews.map((r, index) => {
                const isLast = index === paginatedReviews.length - 1;
                
                return (
                  <tr key={r.id} className="admin-table-row" style={{ 
                    ...
                    <td style={{ padding: '16px 24px' }}>
                      <div style={{ fontWeight: 600, color: 'var(--charcoal, #2a3a30)', fontSize: '0.9rem' }}>{r.name}</div>
                      <div style={{ fontSize: '0.75rem', color: 'rgba(0,0,0,0.5)', marginTop: '2px' }}>{r.email}</div>
                    </td>
                    <td style={{ padding: '16px 24px' }}>
                      <div style={{ color: 'var(--gold, #c8701a)', letterSpacing: '2px' }}>{'★'.repeat(r.rating) + '☆'.repeat(5 - r.rating)}</div>
                      <div style={{ fontSize: '0.75rem', color: 'rgba(0,0,0,0.5)', marginTop: '4px' }}>{r.projectType}</div>
                    </td>
                    <td style={{ padding: '16px 24px' }}>
                      <div style={{ 
                        fontSize: '0.875rem', 
        ...
        {totalCount > 0 && (
          <div style={{ 
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              {page > 1 ? (
                <Link href={`/admin/dashboard/reviews?page=${page - 1}`} className="admin-page-btn" style={{
                  padding: '6px 12px',
                  fontSize: '0.8rem',
                  fontWeight: 500,
                  color: 'var(--charcoal, #2a3a30)',
